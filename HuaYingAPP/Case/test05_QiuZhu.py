...
class QiuZhuTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.s=QiuZhu.QiuZhu(self.driver)
        sleep(10)

    def testQiuZhu(self):
        pp=self.s
        excel_path=Get_ExcelPath_SheetName.Get_ExcelPath_SheetName().getExcelPath("F:\\Community_DD\\Data\\qiuzhu.xls","excelpath_sheetname")
        sheet_name_list=Get_ExcelPath_SheetName.Get_ExcelPath_SheetName().GetSheetName("F:\\Community_DD\\Data\\qiuzhu.xls","excelpath_sheetname")
    ...
    def assertSuccess(self,driver):
        ...
        sleep(2)
        driver.find_element_by_xpath("//android.widget.TextView[contains(@text,'求助记录')]").click()
        count_jilu=driver.find_elements_by_id("com.dinghe.dingding.community:id/tv_content")
        if count_jilu.__len__()>0:
            print "success submit qiuzhu"
        else:
            print "failed submit qiuzhu"
        driver.keyevent(4)
    def tearDown(self):
        self.driver.quit()
